...
class StageManager:
    def __init__(self, config: StageConfiguration, create_shm: bool = True):
        ...
        self.shm_position = None
        self.shm_config = None
        if create_shm:
            try:
                # Try to open existing shared memory first (for reconnection)
                try:
                    from motors.utils.shared_memory import open_shared_stage_position, open_shared_stage_config
                    self.shm_position, self.position_struct = open_shared_stage_position()
                    self.shm_config = open_shared_stage_config()
                    logger.info("Shared memory reconnected to existing")
                    ...
                    logger.info("Shared memory created new")
                except Exception as e:
                    # Shared memory exists but in bad state, try to recreate
                    logger.warning(f"Shared memory reconnect failed, creating new: {e}")
                    try:
                        self.shm_position, self.position_struct = create_shared_stage_position()
                        self.shm_config = create_shared_stage_config()
                    except FileExistsError:
                        # Race condition - exists now, open it
                        self.shm_position, self.position_struct = open_shared_stage_position()
                        self.shm_config = open_shared_stage_config()
    ...
    async def startup(self):
        ...
        if self._is_running:
            logger.warning("Stage manager already running")
            return
        ...
        if self.create_shm:
            logger.info("Starting position monitor task...")
            self._position_task = asyncio.create_task(self._position_monitor_loop())
            logger.info(f"Position monitor task created: {self._position_task}")
        else:
            logger.warning("Shared memory disabled, position monitor will not start")
    ...
    async def shutdown(self):
        ...
        # Don't touch shared memory at all - leave references intact
        # GUI is still using it, and Python's GC will cause BufferError if we
        # try to close or even clear references while other processes are attached.
        # The OS will clean up shared memory when all processes terminate.
    ...
    async def _position_monitor_loop(self):
        ...
        while self._is_running:
            try:
                if not self.motors:
                    logger.debug("Position monitor: No motors yet")
                    ...
                    try:
                        ...
                        if pos:
                            ...
                            if self.create_shm:
                                ...
                                logger.debug(f"Updated {axis.name} position to {pos.actual} um")
                                del sp
                                del raw
                                shm.close()
                    except Exception as e:
