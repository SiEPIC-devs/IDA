...
# ===========================================================================
# Configure root logger ONCE at startup to prevent duplicate log messages
# This ensures all logging.info() etc. calls go to ONE handler only
# ===========================================================================
...
if platform.system() == "Windows":
    try:
        ...
        FILE_LOG.write_line("[OK] Windows Job Object created for process tree management")
...
def start_gui(path: pathlib.Path):
    ...
    if platform.system() == "Windows" and _hJob:
        try:
            ...
            FILE_LOG.write_line(f"[OK] Process {proc.pid} assigned to job object")
        except Exception as e:
            FILE_LOG.write_line(f"! Failed to assign process {proc.pid} to job: {e}")
    
    processes.append(proc)
    FILE_LOG.write_line(f"[START] {path.name} started (pid={proc.pid})")
    ...
    FILE_LOG.write_line(f"[EXIT] {path.name} exited (rc={rc})")

def terminate_all():
    """Enhanced termination with escalating strategies"""
    FILE_LOG.write_line("[STOP] Terminating all subprocesses...")
        ...
        if proc.poll() is None:  # Still running
            try:
                if platform.system() == "Windows":
                    ...
                    proc.terminate()
                FILE_LOG.write_line(f"[OK] Sent termination signal to PID {proc.pid}")
            except Exception as e:
    ...
    if remaining:
        ...
        for proc in remaining:
            try:
                proc.terminate()
                FILE_LOG.write_line(f"[OK] Force terminated PID {proc.pid}")
            except Exception as e:
    ...
    if still_remaining:
        ...
        for proc in still_remaining:
            try:
                proc.kill()
                FILE_LOG.write_line(f"[OK] Killed PID {proc.pid}")
            except Exception as e:
    ...
    if final_remaining:
        ...
        FILE_LOG.write_line("[OK] All processes terminated successfully")
    
    # Close log
    FILE_LOG.close()

def _reset_connection_state():
    """Clear stale device connection config from shared_memory.json on startup.

    Without this, a previous session's Configuration.sensor / tec / smu
    values survive across restarts and cause devices to auto-connect
    before the user clicks "Connect".
    """
    sm_path = GUI_DIR / "database" / "shared_memory.json"
    try:
        import json as _json
        with open(sm_path, "r", encoding="utf-8") as f:
            data = _json.load(f)
        changed = False
        for key in ("stage", "sensor", "tec", "smu"):
            if data.get("Configuration", {}).get(key, "") != "":
                data["Configuration"][key] = ""
                changed = True
        for key in ("stage", "sensor", "tec"):
            if data.get("Configuration_check", {}).get(key, 0) != 0:
                data["Configuration_check"][key] = 0
                changed = True
        if changed:
            with open(sm_path, "w", encoding="utf-8") as f:
                _json.dump(data, f, indent=2)
            print("[runner] Cleared stale connection state from shared_memory.json")
    except Exception as e:
        print(f"[runner] Could not reset connection state: {e}")
...
def main():
    ...
    # Prevent devices from auto-connecting due to leftover config
...
if platform.system() == "Windows":
    ...
    try:
        ...
        FILE_LOG.write_line("[OK] Console close handler registered")
    except Exception as e:
