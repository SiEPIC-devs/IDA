...
class TSPSolver:
    ...
    def _load_selected_numbers(self) -> None:
        # Use thread-safe SharedMemory class to avoid conflicts
        # with other GUI components writing to the same file
        try:
            data = SharedMemory.read({})
            self.selected_numbers = set(data.get("Selection", []))
        except Exception as e:
            print(f"X Failed to read shared_memory: {e}")
            self.selected_numbers = set()

    def _load_coordinates(self) -> None:
        # Retry mechanism for reading coordinates.json
        raw = None
        for attempt in range(5):
            try:
                ...
                if attempt < 4:
                    ...
                    continue
                print(f"X Failed to read coordinates.json: {e}")
                raw = {"_default": {}}
